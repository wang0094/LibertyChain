...
for dir in \
  ...
  "$DIR/../../build/Windows/master/release/bin"
do
  if test -x "$dir/LibertyChaind"
  then
    LibertyChaind="$dir/LibertyChaind"
    break
  fi
done
if test -z "$LibertyChaind"
then
  echo "LibertyChaind not found"
  exit 1
fi
echo "Found: $LibertyChaind"

TORDIR="$DIR/LibertyChain-over-tor"
TORRC="$TORDIR/torrc"
...
if test "$ready" = 0
then
  echo "Error starting LibertyChaind"
  tail -n 400 "$HOME/.bitLibertyChain/bitLibertyChain.log" | grep -Ev stacktrace\|"Error: Couldn't connect to daemon:"\|"src/daemon/main.cpp:.*LibertyChain\ \'" | tail -n 20
  exit 1
fi
