# The Current/Future Status of ZMQ in LibertyChain

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. LibertyChain
...
matching supported by both LibertyChain and ZMQ. The `format`, `context` and `event`
...
The LibertyChain daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The LibertyChain daemon also ensures that
...
The LibertyChain daemon will send a `txpool_add` pub exactly once for each
