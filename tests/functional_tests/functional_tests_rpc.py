...
# a main offline LibertyChaind, does most of the tests
# a restricted RPC LibertyChaind setup with RPC payment
# two local online LibertyChainds connected to each other
N_LibertyChainDS = 4

# 4 wallets connected to the main offline LibertyChaind
# a wallet connected to the first local online LibertyChaind
N_WALLETS = 5
...
LibertyChaind_base = [builddir + "/bin/LibertyChaind", "--regtest", "--fixed-difficulty", str(DIFFICULTY), "--no-igd", "--p2p-bind-port", "LibertyChaind_p2p_port", "--rpc-bind-port", "LibertyChaind_rpc_port", "--zmq-rpc-bind-port", "LibertyChaind_zmq_port", "--non-interactive", "--disable-dns-checkpoints", "--check-updates", "disabled", "--rpc-ssl", "disabled", "--data-dir", "LibertyChaind_data_dir", "--log-level", "1"]
LibertyChaind_extra = [
  ["--offline"],
...
wallet_base = [builddir + "/bin/LibertyChain-wallet-rpc", "--wallet-dir", WALLET_DIRECTORY, "--rpc-bind-port", "wallet_port", "--disable-rpc-login", "--rpc-ssl", "disabled", "--daemon-ssl", "disabled", "--log-level", "1"]
wallet_extra = [
  ["--daemon-port", "18180"],
  ["--daemon-port", "18180"],
  ["--daemon-port", "18180"],
...
for i in range(N_LibertyChainDS):
  command_lines.append([str(18180+i) if x == "LibertyChaind_rpc_port" else str(18280+i) if x == "LibertyChaind_p2p_port" else str(18380+i) if x == "LibertyChaind_zmq_port" else builddir + "/functional-tests-directory/LibertyChaind" + str(i) if x == "LibertyChaind_data_dir" else x for x in LibertyChaind_base])
  if i < len(LibertyChaind_extra):
    command_lines[-1] += LibertyChaind_extra[i]
  outputs.append(open(FUNCTIONAL_TESTS_DIRECTORY + '/LibertyChaind' + str(i) + '.log', 'a+'))
