...
Core tests take longer than any other LibertyChain tests, due to the high amount of computational work involved in validating core components.
...
To run only LibertyChain's core tests (after building):
...
To run only LibertyChain's crypto tests (after building):
...
```bash
LibertyChaind --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `LibertyChain-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only LibertyChain's hash tests (after building):
...
To run only LibertyChain's performance tests (after building):
...
To run only LibertyChain's unit tests (after building):
